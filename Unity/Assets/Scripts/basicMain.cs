...
public class basicMain : MonoBehaviour
{
    ...
    private IEnumerator GetBasic(string url, System.Action<string> callback)
    {
        ...
        if(webRequest.result == UnityWebRequest.Result.ConnectionError          // 결과 값이 접속 오류일 때
            || webRequest.result == UnityWebRequest.Result.ProtocolError)       // 프로토콜 오류일 때
        {
            Debug.Log("네트워크 환경이 좋지 않아서 통신 불가 ");                // 통신 안 됨 예외 처리한다.
        }
        else
        {

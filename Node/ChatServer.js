...
class GameServer{
        ...
        this.wss.on('connection', (socket) => {
            ...
            socket.on('message', (message) =>
            {
                try
                {
                    ...
                    this.brodcast(updateMsg, socket);
                    }
                }
                catch
                {
    ...
    brodcast(data , excludeSocket = null)
    {
        ...
        {
            if (client !== excludeSocket && client.readyState === WebSocket.OPEN)   // ===는 비교 전에 암시적인 형 변환을 하지 않음 (값이나 타입 중 하나라도 다르면 false 반환)
            {
                client.send(message);
                sentCount++;
            }

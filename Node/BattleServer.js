...
class BattleServer {
    constructor(port){
        ...
        this.players = new Map();                    // 데이터와 키를 같이 저장할 수 있는 자료 구조 (JavaScripts)
    ...
    handleMessage(playerId, data)
    {
        ...
        switch (data.type)
        {
            ...
            default :
                console.log(`알 수 없는 메세지 타임 : ${data.type}`);
        }
    }
    ...
    startBattle (player1Id, player2Id)
    {
        ...
        const battleStartMsg = {
            ...
            this.sendToPlayer(player2Id, {
                ...battleStartMsg,                  // JavaScript의 스프레드 문법 (배열이나 문자열을 개별 요소로 분해하여 결합하는 문법)
                oppoent: player1LastAction.name,
                yourTurn : true,
                isPlayer1 : true
            });
        }
        ...
        handleDisconnect(playerId)
        {
            ...
            if (player && player.inBattle)
            {
                ...
                if (battle)
                {
                    ...
                    // 상대방 상태 초기화
                    const oppoent = this.players.get(opponentId);
                    if (oppoent)
                    {
